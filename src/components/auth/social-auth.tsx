...
// Cryptographically secure PKCE Helper Functions
...
export function SocialAuth() {
    ...
    const handleOAuthLogin = async (provider: 'google' | 'github') => {
        try {
            ...
            // 1. Generate PKCE parameters and cryptograhpic state
            const state = generateRandomString(32);
            const codeVerifier = generateRandomString(64);
            const codeChallenge = await generateCodeChallenge(codeVerifier);

            // 2. Pin to sessionStorage (tab-specific, short-lived)
            sessionStorage.setItem('oauth_state', state);
            sessionStorage.setItem('oauth_code_verifier', codeVerifier);
            sessionStorage.setItem('oauth_provider', provider);

            // 3. Construct Authorization URL (Using backend endpoint or strict OAuth provider endpoint)
            const backendBaseUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';

            window.location.href = `${backendBaseUrl}/api/v1/auth/${provider}?state=${state}&code_challenge=${codeChallenge}&code_challenge_method=S256`;
